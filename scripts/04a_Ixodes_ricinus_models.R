...
# highly correlated.)
predictors <- names(I_ricinus_occ_env[, c(7, 11:19)])
...
background_presence_ratio <- round(sum(I_ricinus_occ_env$occ == 0) / sum(I_ricinus_occ_env$occ == 1), 0)
...
I_ricinus_occ_env$abs_index[I_ricinus_occ_env$occ!=1] <- sample(1:background_presence_ratio, sum(I_ricinus_occ_env$occ!=1), replace = TRUE) # Insert sampled numbers of background-presence-ratio
...
for (m in seq_along(my_preds_list)) { # Start the loop over the three different predictor combinations
  ...
  m_rf <- lapply(1:background_presence_ratio, FUN=function(i){sp_train <- I_ricinus_occ_env[c(presences, which(I_ricinus_occ_env$abs_index == i)),]; print(i); randomForest(as.formula(paste('occ~',paste(my_preds, collapse='+'))), 
