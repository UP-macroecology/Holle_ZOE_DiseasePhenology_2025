...
operations <- c("95th percentile", "mean")

# Create a vector containing the different algorithms and their ensemble
alg <- c("ens", "glm", "gam", "rf", "brt")
...
for (o in operations) { # Loop over the 95th percentile and mean functions
  ...
  for (a in alg) { # Loop over the different applied algorithms and their ensemble
    ...
    for (s in env_scenarios) { # Start of the loop over respective environmental change scenario
      ...
      df_long_fut_clim_landuse <- df_long_fut_clim_landuse %>%
        mutate(scenario = "Climate + Land use change",
               lat_band = lat_bands_fut_clim_landuse$lat_band)
      ...
      df_summary_bands <- combined_df %>%
        ...
        summarise(occ_probability = if (o == "95th percentile") quantile(probability, 0.95, na.rm = TRUE)
                  else if (o == "mean") mean(probability, na.rm = TRUE),
                  .groups = "drop")
  ...
  } # Close the loop over the different algorithms and their ensemble
