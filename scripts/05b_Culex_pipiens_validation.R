...
# ---------------------------------------------------------------------- #
#                 05b. Model validation - Culex pipiens                  #
# ---------------------------------------------------------------------- #

# What is done within this script:

# We evaluate the model performance of all algorithms for Culex pipiens using a 
...
# Read in SDM models
load("output_data/models/C_pipiens_SDMs.RData")
...
# Create a matrix to store the cross-validated predictions of the three models based on different predictor sets
m_glm_preds_cv_all <- matrix(nrow=nrow(C_pipiens_occ_env), ncol=length(models_glm))
...
for (glm_index in 1:length(models_glm)) { # Start of the loop over all three models
  ...
  m_glm_perf_cv <- evalSDM(C_pipiens_occ_env$occ, m_glm_preds_cv, weights = weights)
  ...
  absences <- which(C_pipiens_occ_env$occ == 0) # Extract the position index of background data
...
# Create a matrix to store the cross-validated predictions of the three models based on different predictor sets
m_gam_preds_cv_all <- matrix(nrow=nrow(C_pipiens_occ_env), ncol=length(models_gam))
...
for (gam_index in 1:length(models_gam)) { # Start of the loop over all three models
  ...
  # Calculate performance measures
  m_gam_perf_cv <- evalSDM(C_pipiens_occ_env$occ, m_gam_preds_cv, weights = weights)
...
# Create a matrix to store the averaged cross-validated predictions of the three models based on different predictor sets
m_rf_preds_cv_all <- matrix(nrow=nrow(C_pipiens_occ_env), ncol=length(models_rf))
...
for (rf_index in 1:length(models_rf)) { # Start of the loop over all models
  ...
  m_rf_preds_matrix_cv <- matrix(nrow=nrow(C_pipiens_occ_env), ncol=background_presence_ratio)
  ...
  for(i in seq_len(kfolds)){ # Start of the loop over k folds
    ...
    for (y in 1:background_presence_ratio) { # Start the loop over 10 models
      ...
      data_index <- c(presences, which(C_pipiens_occ_env$abs_index == y))
  ...
  # Calculate performance measures
  m_rf_perf_cv <- evalSDM(C_pipiens_occ_env$occ, m_rf_preds_cv, weights = weights)
...
m_brt_preds_cv_all <- matrix(nrow=nrow(C_pipiens_occ_env), ncol=length(models_brt))

for (brt_index in 1:length(models_brt)) { # Start of the loop over all models
  
  print(paste("Evaluating BRT", brt_index))
  
  m_brt_preds_matrix_cv <- matrix(nrow=nrow(C_pipiens_occ_env), ncol=background_presence_ratio)
  ...
  # Calculate performance measures
  m_brt_perf_cv <- evalSDM(C_pipiens_occ_env$occ, m_brt_preds_cv, weights = weights)
...
# Calculate the average cross-validated predictions of the three models with 
# different predictor sets per algorithm
...
m_ens_perf_cv <- evalSDM(C_pipiens_occ_env$occ, m_ens_preds_cv, weights = weights)
...
# for each month
C_pipiens_monthly_validation <- data.frame(matrix(ncol = 10, nrow = 0))
colnames(C_pipiens_monthly_validation) <- c("Month", "AUC", "TSS", "Kappa", "Sens", "Spec", "PCC", "D2", "thresh", "Boyce")
...
for (m in month) { # Start of the loop over all months
  ...
  m_ens_perf_cv_month <- evalSDM(C_pipiens_occ_env_month$occ, monthly_data, weights = monthly_weights)
  ...
  C_pipiens_monthly_validation <- rbind(C_pipiens_monthly_validation, m_ens_perf_cv_month)
...
save(C_pipiens_monthly_validation,  file = "output_data/validation/C_pipiens_monthly_validation.RData")
...
C_pipiens_response_data <- data.frame(matrix(ncol = 4, nrow = 0))
colnames(C_pipiens_response_data) <- c("environmental_values", "predicted_values", "species", "predictor")
...
for (p in my_preds_all) { # Loop through all predictor variables
  ...
  dummy_data <- data.frame(seq(min(C_pipiens_occ_env[,p], na.rm = TRUE), max(C_pipiens_occ_env[,p], na.rm = TRUE), length = 100))
  ...
  plot_response$species <- "Culex pipiens"
  ...
  C_pipiens_response_data <- rbind(C_pipiens_response_data, plot_response)
  ...
  ggsave(paste0("output_data/plots/response_curves/C_pipiens_response_curve_smoothed_",p,".png"), width = 5, height = 5, units = "cm")
...
save(C_pipiens_response_data, file = "output_data/validation/C_pipiens_response_data.RData")
