...
# ---------------------------------------------------------------------- #
#                 08a. Absence data preparation - TBE                    #
# ---------------------------------------------------------------------- #
...
# occurrence probabilities of the main vector species, Ixodes ricinus.
...
r_past_preds_clim_landuse <- terra::rast("output_data/results/preprocessed_predictions/Ixodes_ricinus/I_ricinus_preds_clim_landuse_ens_1970_2019.tif") # Ensemble predictions of occurrence probability (under factual climate and land use) of main vector species Ixodes ricinus
...
TBE_infection_data <- read.csv("input_data/raw_infection_data/TBE.csv") # raw TBE infection data provided by ECDC/TESSy
...
# Format the months within the data frames of cleaned and raw infection data
...
TBE_infection_data$DateUsedForStatisticsMonth <- sprintf("%02d", as.numeric(TBE_infection_data$DateUsedForStatisticsMonth))
...
# Create a vector that contains the two-letter iso codes of the EU/EEA member 
# states (If not identical with the NUTS3 country code, also add that one; 
...
# Starting with 2011 as this is the first year with reported infections after
# occurrence cleaning
ECDC_nonreporting_countries_2011 <- c("BE", "DK", "FR", "IT", # countries with surveillance system but not mandatory (or not well defined)
                                      "BG", "CY", "HR", "IS", "IE", "LU", "LI", # countries have no developed surveillance system
                                      "MT", "NL", "PT", "ES", "UK", "GB")       # countries have no developed surveillance system 

ECDC_nonreporting_countries_2012 <- c("BG", "CY", "DK", "IS", "IT", "LI", "LU", "MT", "NL", "PT")

ECDC_nonreporting_countries_2013 <- c("BG", "CY", "DK", "IS", "LI", "LU", "MT", "NL", "PT")

ECDC_nonreporting_countries_2014 <- c("CY", "DK", "IS", "LI", "MT", "NL", "PT")

ECDC_nonreporting_countries_2015 <- c("CY", "DK", "IS", "LI", "MT", "NL", "PT")

ECDC_nonreporting_countries_2016 <- c("CY", "DK", "IS", "LI", "MT", "PT")

ECDC_nonreporting_countries_2017 <- c("CY", "DK", "IS", "LI", "MT", "PT")

ECDC_nonreporting_countries_2018 <- c("CY", "IS", "LI", "MT", "PT")

ECDC_nonreporting_countries_2019 <- c("CY", "IS", "LI", "MT", "PT")

# Special cases: Austria, France, and Norway did report but consistently on NUTS2 or 
# country level - therefore these countries are fully eligible for absence 
# generation if no cases were reported. Netherlands did not report on NUTS3 level
# at all but since NL only started to report in 2016, they also form part of the
# fully non-reporting countries until then
countries_no_nuts3 <- c("AT", "FR", "NO", "NL")
...
# remove duplicates in 0.5° cells, generate absence data,
...
for (d in date_sequence) { # Start of the loop over all dates
  ...
  subset_year_month <- TBE_occurrences_cleaned[
    TBE_occurrences_cleaned$year  == y & 
      TBE_occurrences_cleaned$month == m, 
  ]
...
# 4. Generation of absence data ---------------------------------------------
    ...
    # Extract the countries that consistently did not report on NUTS3
    # level but reported a presence within that month
    subset_year_month_all_raw <- TBE_infection_data[TBE_infection_data$DateUsedForStatisticsYear == y & 
                                                      TBE_infection_data$DateUsedForStatisticsMonth == m, ]
    
    subset_year_month_all_raw_country <- unique(subset_year_month_all_raw$ReportingCountry[
      subset_year_month_all_raw$ReportingCountry %in% countries_no_nuts3])
    
    # Make sure municipalities within this country are not eligible for absence 
    # generation
    ECDC_reporting_countries_year <- setdiff(ECDC_reporting_countries_year, subset_year_month_all_raw_country)
...
# 5. Spatial thinning of the presence and absence data -------------------------
    ...
    # Merge data frames to only retain thinned absences
...
# 6. Join with environmental data  ---------------------------------------------
    ...
    Species_data <- r_past_preds_clim_landuse[[paste0(m,"/",y)]] # Occurrence probabilities of the vector species Ixodes ricinus (under factual climate and land use)
