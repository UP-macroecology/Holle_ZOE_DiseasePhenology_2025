...
for (o in operations) { # Loop over the 95th percentile and mean functions
  ...
  if (o == "95th percentile") { fun <- percentile_95 
  ...
  monthly_clim_landuse <- global(r_curr_preds_clim_landuse, fun = fun)
  monthly_clim_landuse <- data.frame(date = dates, occurrence = monthly_clim_landuse[,1], scenario = "Factual")
  
  monthly_noclim_landuse <- global(r_curr_preds_noclim_landuse, fun = fun)
...
for (s in env_scenarios) { # Start of the loop over respective environmental change scenario
  ...
  for (o in operations) { # Loop over the 95th percentile and mean functions
    ...
    monthly_clim_landuse <- global(r_fut_preds_clim_landuse, fun = fun)
    monthly_clim_landuse <- data.frame(date = dates, occurrence = monthly_clim_landuse[,1], scenario = "Climate + Land use change")
    
    monthly_clim_nolanduse <- global(r_fut_preds_clim_nolanduse, fun = fun)
