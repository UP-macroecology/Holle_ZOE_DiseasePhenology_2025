...
# ---------------------------------------------------------------------- #
#                 05a. Model validation - Ixodes ricinus                 #
# ---------------------------------------------------------------------- #

# What is done within this script:

# We evaluate the model performance of all algorithms for Ixodes ricinus using a 
...
m_glm_preds_cv_all <- matrix(nrow=nrow(I_ricinus_occ_env), ncol=length(models_glm))
...
for (glm_index in 1:length(models_glm)) { # Start of the loop over all three models
  ...
  m_glm_perf_cv <- evalSDM(I_ricinus_occ_env$occ, m_glm_preds_cv, weights = weights)
  ...
  absences <- which(I_ricinus_occ_env$occ == 0) # Extract the position index of background data
...
# Create a matrix to store the cross-validated predictions of the three models
m_gam_preds_cv_all <- matrix(nrow=nrow(I_ricinus_occ_env), ncol=length(models_gam))
...
for (gam_index in 1:length(models_gam)) { # Start of the loop over all three models
  ...
  # Calculate performance measures
  m_gam_perf_cv <- evalSDM(I_ricinus_occ_env$occ, m_gam_preds_cv, weights = weights)
...
# Create a matrix to store the cross-validated predictions of the three models
m_rf_preds_cv_all <- matrix(nrow=nrow(I_ricinus_occ_env), ncol=length(models_rf))
...
for (rf_index in 1:length(models_rf)) { # Start of the loop over all models
  ...
  m_rf_preds_matrix_cv <- matrix(nrow=nrow(I_ricinus_occ_env), ncol=background_presence_ratio)
  ...
  # Calculate performance measures
  m_rf_perf_cv <- evalSDM(I_ricinus_occ_env$occ, m_rf_preds_cv, weights = weights)
...
# Create a matrix to store the cross-validated predictions of the three models
m_brt_preds_cv_all <- matrix(nrow=nrow(I_ricinus_occ_env), ncol=length(models_brt))

for (brt_index in 1:length(models_brt)) { # Start of the loop over all models
  
  print(paste("Evaluating BRT", brt_index))
  
  m_brt_preds_matrix_cv <- matrix(nrow=nrow(I_ricinus_occ_env), ncol=background_presence_ratio)
  ...
  # Calculate performance measures
  m_brt_perf_cv <- evalSDM(I_ricinus_occ_env$occ, m_brt_preds_cv, weights = weights)
...
m_ens_perf_cv <- evalSDM(I_ricinus_occ_env$occ, m_ens_preds_cv, weights = weights)
m_ens_preds_cv_presences <- m_ens_preds_cv[presences]
absences <- which(I_ricinus_occ_env$occ == 0)
...
save(m_glm_preds_cv_all, glm_performances, m_gam_preds_cv_all, gam_performances,
     ...
     file = "output_data/validation/I_ricinus_validation.RData")
...
# for each month
I_ricinus_monthly_validation <- data.frame(matrix(ncol = 10, nrow = 0))
colnames(I_ricinus_monthly_validation) <- c("Month", "AUC", "TSS", "Kappa", "Sens", "Spec", "PCC", "D2", "thresh", "Boyce")
...
for (m in month) { # Start of the loop over all months
  ...
  I_ricinus_occ_env_month <- subset(I_ricinus_occ_env, I_ricinus_occ_env$month == m)
  
  # Get the indices of the presences and absences of the respective month
  monthly_data_indices <- which(I_ricinus_occ_env$month == m)
  ...
  m_ens_perf_cv_month <- evalSDM(I_ricinus_occ_env_month$occ, monthly_data, weights = monthly_weights)
  
  # Calculate the Boyce index with smoothing methods for the respective month
  presences_month <- which(I_ricinus_occ_env_month$occ == 1)
  m_ens_preds_cv_presences_month <- monthly_data[presences_month]
  absences_month <- which(I_ricinus_occ_env_month$occ == 0)
  ...
  I_ricinus_monthly_validation <- rbind(I_ricinus_monthly_validation, m_ens_perf_cv_month)
...
save(I_ricinus_monthly_validation,  file = "output_data/validation/I_ricinus_monthly_validation.RData")
...
for (p in my_preds_all) { # Loop through all predictor variables
  ...
  dummy_data <- data.frame(seq(min(I_ricinus_occ_env[,p], na.rm = TRUE), max(I_ricinus_occ_env[,p], na.rm = TRUE), length = 100))
  for (pred in my_preds_minus_p) {
    dummy_data[[pred]] <- mean(I_ricinus_occ_env[[pred]], na.rm = TRUE)
  }
  ...
  ggsave(paste0("output_data/plots/response_curves/I_ricinus_response_curve_smoothed_",p,".png"), width = 5, height = 5, units = "cm")
