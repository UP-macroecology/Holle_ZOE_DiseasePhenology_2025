...
absences <- which(I_ricinus_occ_env$occ == 0) # Extract the position index of presences
...
# RF
print("RF")
m_rf_preds_matrix_cv <- matrix(nrow=nrow(I_ricinus_occ_env), ncol=ratio_presence_background)
for(i in seq_len(kfolds)){ # Start of the loop over k folds
  print(i)
  hold_in <- which(ks != i)
  hold_out <- which(ks == i)
...
# BRT
print("BRT")
m_brt_preds_matrix_cv <- matrix(nrow=nrow(I_ricinus_occ_env), ncol=ratio_presence_background)
for(i in seq_len(kfolds)){ # Start of the loop over k folds
  print(i)
  hold_in <- which(ks != i)
  hold_out <- which(ks == i)
...
# # Maxent
# print("Maxent")
# m_maxent_preds_matrix_cv <- matrix(nrow=nrow(I_ricinus_occ_env), ncol=10)
# for(i in seq_len(kfolds)){ # loop over k folds
#   print(i)
#   hold_in <- which(ks != i)
#   hold_out <- which(ks == i)
# 
#   for (y in 1:10) { # loop over 10 models
#     print(y)
#     data_index <- c(presences, which(I_ricinus_occ_env$abs_index == y))
# 
#     cv_train <- I_ricinus_occ_env[hold_in[hold_in %in% data_index],]
#     cv_test <- I_ricinus_occ_env[hold_out,]
# 
#     cv_maxent <- maxnet::maxnet(p= cv_train$occ, data= cv_train[,my_preds])
#     m_maxent_preds_matrix_cv[hold_out,y] <- predict(cv_maxent, cv_test, type='logistic')
#   }
# }
# 
# m_maxent_preds_cv <- rowMeans(m_maxent_preds_matrix_cv)
# 
# # Calculate performance measures
# m_maxent_perf_cv <- evalSDM(I_ricinus_occ_env$occ, m_maxent_preds_cv)
# 
# # Calculate Boyce index with smoothing methods
# m_maxent_preds_cv_presences <- m_maxent_preds_cv[presences] # Just retain the predictions of the presences based on indices
# absences <- which(I_ricinus_occ_env$occ == 0) # Extract the position index of presences
# m_maxent_preds_cv_absences <- m_maxent_preds_cv[absences] # Just retain the predictions of the absences based on indices
# m_maxent_boyce_cv <- sfbi(m_maxent_preds_cv_presences, m_maxent_preds_cv_absences, ktry = 10) # Apply sfbi function
# 
# boyce_index_m_maxent <- m_maxent_boyce_cv[6] # Extract values of smoothed Boyce index using the mean of all smoothers
# 
# # Add the Boyce index to the performance metrics data frame
# m_maxent_perf_cv$Boyce <- boyce_index_m_maxent
...
m_ens_perf_cv <- evalSDM(I_ricinus_occ_env$occ, m_ens_preds_cv)
...
for (m in month) { # Start of the loop over all months
  ...
  m_ens_perf_cv_month <- evalSDM(I_ricinus_occ_env_month$occ, monthly_data)
  ...
  save(m_ens_perf_cv_month,  file = paste0("output_data/validation/I_ricinus_monthly_validation_",m,".RData"))
...
for (m in my_preds) { # Loop through the predictor variables
  ...
  dummy_data <- data.frame(seq(min(I_ricinus_occ_env[,m], na.rm = TRUE), max(I_ricinus_occ_env[,m], na.rm = TRUE), length = 100))
  ...
  print("RF")
  response_preds_rf_all <- matrix(nrow=nrow(dummy_data), ncol=4)
  ...
  print("BRT")
  response_preds_brt_all <- matrix(nrow=nrow(dummy_data), ncol=4)
  ...
  ggsave(paste0("output_data/plots/response_curves/I_ricinus_response_curve_smoothed_",m,".png"), width = 5, height = 5, units = "cm")
