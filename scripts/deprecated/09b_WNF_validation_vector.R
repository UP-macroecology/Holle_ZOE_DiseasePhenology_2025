...
# Read in SDM models
load("output_data/models/WNF_SDMs_vector.RData")
...
save(m_glm_preds_cv_all, glm_performances, m_gam_preds_cv_all, gam_performances,
     ...
     file = "output_data/validation/WNF_validation_vector.RData")
...
load("output_data/models/WNF_SDMs_vector.RData") # The occurrence data frame
load("output_data/validation/WNF_validation_vector.RData") # The cross-validated ensemble predictions
...
for (m in month) { # Start of the loop over all months
  ...
  save(m_ens_perf_cv_month,  file = paste0("output_data/validation/WNF_monthly_validation_",m,"_vector.RData"))
...
# Read in the needed data
load("output_data/models/WNF_SDMs_vector.RData") # The different models and selected predictor variables
...
my_preds_all <- c("tas", "tasmin", "tasmax", "C_pipiens", "hurs", "primary_openland", "secondary_openland", "cropland", "rangeland", "pr", "primary_forest", "pasture")
temp_var <- c("tas", "tasmin", "tasmax")
remain_var <- c("C_pipiens", "hurs", "primary_openland", "secondary_openland", "cropland", "rangeland", "pr", "primary_forest", "pasture")
...
for (p in my_preds_all) { # Loop through all predictor variables
  ...
  ggsave(paste0("output_data/plots/response_curves/WNF_response_curve_smoothed_",p,"_vector.png"), width = 5, height = 5, units = "cm")
