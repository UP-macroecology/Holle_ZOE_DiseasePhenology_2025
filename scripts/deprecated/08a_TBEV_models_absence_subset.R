...
for (t in methods) { # Start the loop of the three methods
  ...
  load(paste0("output_data/data/TBEV_occ_env_absence_",t,".RData"))
...
# Create a subset to have as many absences as presences per month and year in the
...
# Generate weights
wgt <- rep(1, times = nrow(TBEV_occ_env))
...
for (m in seq_along(my_preds_list)) { # Start the loop over the three different predictor combinations
  ...
  m_glm <- step(glm(as.formula(paste('occ~',paste(c(my_preds, paste0('I(',my_preds,'^2)')), collapse ='+'))),
                    family='binomial', data = TBEV_occ_env, weights = wgt))
...
# Save the models
save(models_glm, models_gam, models_rf, models_brt, predictors, my_preds_list, TBEV_occ_env, 
     file = paste0("output_data/models/TBEV_SDMs_absence_subset_",t,".RData"))
