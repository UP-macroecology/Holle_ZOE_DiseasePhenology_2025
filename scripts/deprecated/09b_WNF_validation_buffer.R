...
# Read in SDM models
load("output_data/models/WNF_SDMs_buffer.RData")

load("output_data/models/WNF_SDMs_buffer500_presenceabsence.RData")
...
for (rf_index in 1:length(models_rf)) { # Start of the loop over all models
  ...
  m_rf_preds_matrix_cv <- matrix(nrow=nrow(WNF_occ_env), ncol=background_presence_ratio)
  ...
  for(i in seq_len(kfolds)){ # Start of the loop over k folds
    ...
    for (y in 1:background_presence_ratio) { # Start the loop over 10 models
      ...
      data_index <- c(presences, which(WNF_occ_env$abs_index == y))
      
      cv_train <- WNF_occ_env[hold_in[hold_in %in% data_index],]
      cv_test <- WNF_occ_env[hold_out,]
...
for (brt_index in 1:length(models_brt)) { # Start of the loop over all models
  ...
  m_brt_preds_matrix_cv <- matrix(nrow=nrow(WNF_occ_env), ncol=background_presence_ratio)
...
save(m_glm_preds_cv_all, glm_performances, m_gam_preds_cv_all, gam_performances,
     ...
     file = "output_data/validation/WNF_validation_buffer500_presenceabsence.RData")
...
load("output_data/models/WNF_SDMs_buffer.RData") # The occurrence data frame
load("output_data/validation/WNF_validation_buffer.RData") # The cross-validated ensemble predictions
...
for (m in month) { # Start of the loop over all months
  ...
  save(m_ens_perf_cv_month,  file = paste0("output_data/validation/WNF_monthly_validation_",m,"_buffer.RData"))
...
# Read in the needed data
load("output_data/models/WNF_SDMs_buffer.RData") # The different models and selected predictor variables
...
my_preds_all <- c("tas", "tasmin", "tasmax", "cropland", "hurs", "secondary_openland", "urban", "pr", "rangeland", "primary_forest", "pasture", "primary_openland", "C_pipiens")
temp_var <- c("tas", "tasmin", "tasmax")
remain_var <- c("cropland", "hurs", "secondary_openland", "urban", "pr", "rangeland", "primary_forest", "pasture", "primary_openland", "C_pipiens")
...
for (p in my_preds_all) { # Loop through all predictor variables
  ...
  ggsave(paste0("output_data/plots/response_curves/WNF_response_curve_smoothed_",p,"_buffer500_presenceabsence.png"), width = 5, height = 5, units = "cm")
...
for (p in my_preds_all) { # Loop through all predictor variables
  ...
  ggsave(paste0("output_data/plots/response_curves/WNF_response_scatterplot_",p,"_buffer500_presenceabsence.png"), width = 10, height = 5, units = "cm")
