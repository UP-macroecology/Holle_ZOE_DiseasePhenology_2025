...
# Read in SDM models
load("output_data/models/TBEV_SDMs_vector.RData")
...
save(m_glm_preds_cv_all, glm_performances, m_gam_preds_cv_all, gam_performances,
     ...
     file = "output_data/validation/TBEV_validation_vector.RData")
...
load("output_data/models/TBEV_SDMs_vector.RData") # The occurrence data frame
load("output_data/validation/TBEV_validation_vector.RData") # The cross-validated ensemble predictions
...
for (m in month) { # Start of the loop over all months
  ...
  save(m_ens_perf_cv_month,  file = paste0("output_data/validation/TBEV_monthly_validation_",m,"_vector.RData"))
...
# Read in the needed data
load("output_data/models/TBEV_SDMs_vector.RData") # The different models and selected predictor variables
...
my_preds_all <- c("tas", "tasmin", "tasmax", "I_ricinus", "pasture", "primary_forest", "cropland", "secondary_openland", "hurs", "primary_openland", "rangeland", "urban", "pr")
temp_var <- c("tas", "tasmin", "tasmax")
remain_var <- c("I_ricinus", "pasture", "primary_forest", "cropland", "secondary_openland", "hurs", "primary_openland", "rangeland", "urban", "pr")
...
for (p in my_preds_all) { # Loop through all predictor variables
  ...
  ggsave(paste0("output_data/plots/response_curves/TBEV_response_curve_smoothed_",p,"_vector.png"), width = 5, height = 5, units = "cm")
