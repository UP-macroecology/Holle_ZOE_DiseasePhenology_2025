...
# 2. Generation of background data ---------------------------------------------
    ...
    if (nrow(occ_coords) > 0) { # Just continue if at least one presence of the respective month and year are within the continent of Europe
      ...
      buf_150 <- rasterize(buf_150, eu_eea_mask)
      
      # set raster cells outside the buffer to NA
      buf_150 <- terra::mask(eu_eea_mask, buf_150, overwrite = TRUE)
      ...
      abs_coords_150_all <- as.data.frame(xyFromCell(buf_150, abs_indices_150))
      colnames(abs_coords_150_all) = c("lon", "lat")
      ...
      abs_coords_150_all_points <- vect(abs_coords_150_all, geom = c("lon", "lat"), crs = crs(nuts_3_raster_mask))
      abs_coords_150_all$NUTS_3 <- terra::extract(nuts_3_raster_mask, abs_coords_150_all_points)[,2]
      abs_coords_150_all$country <- substr(abs_coords_150_all$NUTS_3, 1, 2)
      abs_coords_150 <- abs_coords_150_all %>% filter(country %in% ECDC_reporting_countries)
      abs_coords_150 <- abs_coords_150[, c("lon", "lat")]
...
TBEV_occ_env_thresh <- TBEV_occ_env
TBEV_occ_env_nothresh <- TBEV_occ_env
TBEV_occ_env_presenceabsence <- TBEV_occ_env

# Use threshold of the maxSSS to set a relationship of virsu and vector occurrence
TBEV_occ_env_thresh$I_ricinus[TBEV_occ_env_thresh$I_ricinus < threshold] <- 0 # Set values below the threshold to 0
TBEV_occ_env_presenceabsence$I_ricinus[TBEV_occ_env_presenceabsence$I_ricinus < threshold] <- 0 # Set values below the threshold to 0
TBEV_occ_env_presenceabsence$I_ricinus[TBEV_occ_env_presenceabsence$I_ricinus >= threshold] <- 1 # Set values above that threshold to 1
  
  
# Get a summary of presence and background data numbers
table(TBEV_occ_env$occ) # 1:2696, 0: 13495
...
save(TBEV_occ_env_thresh, file = "output_data/data/TBEV_occ_env_buffer_thresh.RData")
save(TBEV_occ_env_nothresh, file = "output_data/data/TBEV_occ_env_buffer_nothresh.RData")
save(TBEV_occ_env_presenceabsence, file = "output_data/data/TBEV_occ_env_buffer_presenceabsence.RData")
