...
# Create a vector with the three thresholding methods
methods <- c("presenceabsence")

# Create a vector with buffer distances used
buffer <- c("100", "500")


for (t in methods) { # Start the loop over methods
  
  print(t)
  
  for (b in buffer) { # Start the loop over buffer distance
    
    print(b)

# Read in presence and background data
load(paste0("output_data/data/WNF_occ_env_buffer",b,"_",t,".RData"))

assign("WNF_occ_env", get(paste0("WNF_occ_env", "_",t)))
...
weights <- ifelse(WNF_occ_env$occ == 1, 1, sum(WNF_occ_env$occ == 1) / sum(WNF_occ_env$occ == 0))
...
background_presence_ratio <- round(sum(WNF_occ_env$occ == 0) / sum(WNF_occ_env$occ == 1), 0)
...
presences <- which(WNF_occ_env$occ == 1) # Extract the position index of presences
WNF_occ_env$abs_index <- NA # Create a new column to insert the absence index
WNF_occ_env$abs_index[WNF_occ_env$occ!=1] <- sample(1:background_presence_ratio, sum(WNF_occ_env$occ!=1), replace = TRUE) # Insert sampled numbers of background-presence-ratio
...
for (m in seq_along(my_preds_list)) { # Start the loop over the three different predictor combinations
  ...
  smooth_preds <- setdiff(my_preds, "C_pipiens")
  
  m_gam <- mgcv::gam(as.formula(paste('occ~',paste(c(paste0('s(', smooth_preds,',k=4)'), 'C_pipiens'), collapse='+'))),
                     family='binomial', data = WNF_occ_env, weights = weights)
...
for (m in seq_along(my_preds_list)) { # Start the loop over the three different predictor combinations
  ...
  m_rf <- lapply(1:background_presence_ratio, FUN=function(i){sp_train <- WNF_occ_env[c(presences, which(WNF_occ_env$abs_index == i)),]; print(i); randomForest(as.formula(paste('occ~',paste(my_preds, collapse='+'))), 
...
for (m in seq_along(my_preds_list)) { # Start the loop over the three different predictor combinations
  ...
  m_brt = lapply(1:background_presence_ratio, FUN=function(i) {
    ...
    LR = 0.01;
    while(opt.LR){
      m.brt <- try(gbm.step(data = WNF_occ_env[c(presences, which(WNF_occ_env$abs_index == i)),], gbm.x = my_preds, gbm.y = "occ", family = 'bernoulli', tree.complexity = 2, bag.fraction = 0.75, learning.rate = LR, verbose=F, plot.main=F))
...
# Save the models
save(models_glm, models_gam, models_rf, models_brt, weights, predictors, my_preds_list, presences, WNF_occ_env, background_presence_ratio, 
     file = paste0("output_data/models/WNF_SDMs_buffer",b,"_",t,".RData"))

} # Close the loop over buffer distance
  
  
} # Close the loop over methods
