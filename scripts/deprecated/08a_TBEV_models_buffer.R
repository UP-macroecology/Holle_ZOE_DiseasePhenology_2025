...
for (t in methods) { # Start the loop of the three methods
  
  print(t)

# Read in presence and background data
load(paste0("output_data/data/TBEV_occ_env_buffer_",t,".RData"))

assign("TBEV_occ_env", get(paste0("TBEV_occ_env", "_",t)))
...
weights <- ifelse(TBEV_occ_env$occ == 1, 1, sum(TBEV_occ_env$occ == 1) / sum(TBEV_occ_env$occ == 0))
...
background_presence_ratio <- round(sum(TBEV_occ_env$occ == 0) / sum(TBEV_occ_env$occ == 1), 0)
...
presences <- which(TBEV_occ_env$occ == 1) # Extract the position index of presences
TBEV_occ_env$abs_index <- NA # Create a new column to insert the absence index
TBEV_occ_env$abs_index[TBEV_occ_env$occ!=1] <- sample(1:background_presence_ratio, sum(TBEV_occ_env$occ!=1), replace = TRUE) # Insert sampled numbers of background-presence-ratio
...
for (m in seq_along(my_preds_list)) { # Start the loop over the three different predictor combinations
  ...
  smooth_preds <- setdiff(my_preds, "I_ricinus")
  
  m_gam <- mgcv::gam(as.formula(paste('occ~',paste(c(paste0('s(', smooth_preds,',k=4)'), 'I_ricinus'), collapse='+'))),
                     family='binomial', data = TBEV_occ_env, weights = weights)
...
for (m in seq_along(my_preds_list)) { # Start the loop over the three different predictor combinations
  ...
  m_rf <- lapply(1:background_presence_ratio, FUN=function(i){sp_train <- TBEV_occ_env[c(presences, which(TBEV_occ_env$abs_index == i)),]; print(i); randomForest(as.formula(paste('occ~',paste(my_preds, collapse='+'))), 
...
for (m in seq_along(my_preds_list)) { # Start the loop over the three different predictor combinations
  ...
  m_brt = lapply(1:background_presence_ratio, FUN=function(i) {
    ...
    LR = 0.01;
    while(opt.LR){
      m.brt <- try(gbm.step(data = TBEV_occ_env[c(presences, which(TBEV_occ_env$abs_index == i)),], gbm.x = my_preds, gbm.y = "occ", family = 'bernoulli', tree.complexity = 2, bag.fraction = 0.75, learning.rate = LR, verbose=F, plot.main=F))
...
# Save the models
save(models_glm, models_gam, models_rf, models_brt, weights, predictors, my_preds_list, presences, TBEV_occ_env, background_presence_ratio, 
     file = paste0("output_data/models/TBEV_SDMs_buffer_",t,".RData"))
