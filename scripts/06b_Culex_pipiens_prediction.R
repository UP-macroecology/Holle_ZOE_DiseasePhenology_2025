...
for (s in scenario) { # Start of the loop over the three different climate forcing scenarios
  ...
  # Prepare a Spatraster to store all prediction rasters of all prediction years, using the env_data as template
  # For all algorithms and their ensemble
  r_fut_preds_clim_nolanduse_ens <- terra::rast(example_data, nlyrs = 492)
  r_fut_preds_clim_nolanduse_ens_bin <- terra::rast(example_data, nlyrs = 492)
  r_fut_preds_clim_nolanduse_glm <- terra::rast(example_data, nlyrs = 492)
  r_fut_preds_clim_nolanduse_gam <- terra::rast(example_data, nlyrs = 492)
  r_fut_preds_clim_nolanduse_rf <- terra::rast(example_data, nlyrs = 492)
  r_fut_preds_clim_nolanduse_brt <- terra::rast(example_data, nlyrs = 492)
  ...
  for (y in years) { # Start of the loop over the prediction years
    ...
    # Stack the all raster for each year
    r_fut_preds_clim_nolanduse_ens <- c(r_fut_preds_clim_nolanduse_ens, r_fut_preds_year_ens)
    r_fut_preds_clim_nolanduse_ens_bin <- c(r_fut_preds_clim_nolanduse_ens_bin, r_fut_preds_year_ens_bin)
    r_fut_preds_clim_nolanduse_glm <- c(r_fut_preds_clim_nolanduse_glm, r_fut_preds_year_glm)
    r_fut_preds_clim_nolanduse_gam <- c(r_fut_preds_clim_nolanduse_gam, r_fut_preds_year_gam)
    r_fut_preds_clim_nolanduse_rf <- c(r_fut_preds_clim_nolanduse_rf, r_fut_preds_year_rf)
    r_fut_preds_clim_nolanduse_brt <- c(r_fut_preds_clim_nolanduse_brt, r_fut_preds_year_brt)
